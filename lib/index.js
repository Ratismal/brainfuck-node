...
class Brainfuck {
    ...
    execute(code, input = '') {
        ...
        while (position.current < code.length) {
            ...
            switch (char) {
                ...
                case '[':
                    if (memory.current != 0) {
                        ...
                        if (cont == true) {
                            throw new Error(`Unmatched loop at index ${position.list[base]}`);
                        }
                    }
                    break;
                case ']':
                    if (position.pointer == 0) {
                        throw new Error(`Unmatched loop at index ${position.current}`);
                    } else {
                        position.decrementPointer();
                        position.decrement();
                    }    
            ...
            position.increment();
            if (++steps == this.maxSteps) throw new Error(`Too many steps (${steps} reached)`);
        }
        const end = Date.now();
        return new Result(output, memory, steps, end - start);
    }
}
